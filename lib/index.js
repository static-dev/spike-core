 ...
 */
module.exports = class Spike extends EventEmitter {
  constructor (opts = {}) {
    super()
   ...
   * @param {String} [options.src=https://github.com/carrot/spike-base.git] - path to the template source
  ...
  static new (options) {
    ...
    const schema = Joi.object().keys({
      ...
      src: Joi.string().default('https://github.com/carrot/spike-base.git'),
      locals: Joi.object(),
      emitter: Joi.func()
    }).with('template', 'src')
    ...
    const tplPath = path.join(os.homedir(), '.spike/templates')
    mkdirp.sync(tplPath)
