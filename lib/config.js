...
// temporary patch while posthtml is fully integrated
...
module.exports = class Config {
  ...
  validateOpts (opts) {
    const schema = Joi.object().keys({
      ...
      posthtml: Joi.object().default().keys({
        plugins: Joi.array().single().default([])
      }),
  ...
  transformSpikeOptionsToWebpack (opts) {
    ...
    this.posthtml = (ctx) => {
      const p = url.parse(ctx.request.split('!').slice(-1)[0]).pathname
      const jadeOpts = Object.assign(opts.locals, opts.jade, { filename: p })
      return { defaults: [jadePlugin(jadeOpts)] }
    }
