...
/**
 * @class PugPlugin
 * @classdesc A webpack plugin that takes in .pug files, adds them to the
...
module.exports = class PugWebpackPlugin {
  ...
  apply (compiler) {
    ...
    // inject pug files into webpack's pipeline
    compiler.plugin('make', (compilation, done) => {
      pugFiles = compiler.options.spike.files.pug
      this.util.addFilesAsWebpackEntries(compilation, pugFiles)
        .done(() => done(), done)
    })
    ...
    compiler.plugin('emit', (compilation, done) => {
      pugFiles.forEach((f) => {
        ...
        const outputPath = this.util.getOutputPath(f).relative.replace(/\.pug$/, '.html')
    ...
    // remove pug assets from webpack pipeline, unless pugTemplates option
    // is present
    compiler.plugin('compilation', (compilation) => {
      compilation.plugin('optimize-chunk-assets', (chunks, done) => {
        this.util.removeAssets(compilation, pugFiles)
        done()
      })
    })
  }
