...
export function getFilesFromGlob (opts) {
  ...
  files.all = glob.sync(matcher, { ignore: opts.ignore })

  for (let key in opts.matchers) {
    files[key] = micromatch(files.all, opts.matchers[key], { dot: true })
  }
  return files
}
...
export function addFilesAsWebpackEntries (files, opts, compilation) {
  return W.all(files.map((f) => {
    const name = this.getOutputPath(f, opts)
    const relativePath = f.replace(opts.root, '.')
    const dep = new SingleEntryDependency(relativePath)
 ...
 */
export function removeAssets (compilation, opts, done) {
  for (let key in opts.matchers) {
    for (const a in compilation.assets) {
      // this seems question, why magic number 3?
