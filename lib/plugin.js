...
module.exports = class SpikeWebpackPlugin {
  ...
  sortFiles (compiler, compilation, done) {
    // TODO this.util.getSpikeOpts()
    const spikeOpt = compiler.options.plugins.find((p) => p.id === 'spikeOptions').options
    // first we load up and categorize all the files in the project
    spikeOpt.files = this.getFilesFromGlob()
    ...
    compiler.options.module.rules.map((rule) => {
      if (!rule.options._core) { return }
      rule.test = pathToRegex(spikeOpt.files[rule.options._core])
    })
