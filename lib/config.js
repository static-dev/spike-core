  ...
  validateOpts (opts) {
    const schema = Joi.object().keys({
      ...
      server: Joi.object().default().keys({
        contentBase: Joi.string().default(opts.root),
        port: Joi.number().default(1111),
        hot: Joi.boolean().default(false),
        historyApiFallback: Joi.boolean().default(false),
        proxy: Joi.object(),
        quiet: Joi.boolean().default(true),
        noInfo: Joi.boolean().default(true),
    ...
    // default values for server options that are too much for Joi to handle
    if (!res.server.publicPath) {
      res.server.publicPath = path.join(res.root, res.outputDir)
    }
