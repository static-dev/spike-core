...
module.exports = class SpikeWebpackPlugin {
  ...
  getFilesFromGlob () {
    ...
    // TODO skipSpikeProcessing is BROKEN here due to webpack validation
    const skipLoaderTests = util.conf.module.rules.reduce((m, l) => {
      if (!l._core && l.skipSpikeProcessing) m.push(l.test)
      return m
    }, [])
    ...
    util.conf.module.rules.filter((l) => l.extension).map((l) => {
