...
module.exports = class Config {
  ...
  validateOpts (opts) {
    ...
    res.server.files = [{
      ...
      fn: (event, file) => {
        ...
        const opts = this.util.conf.plugins.find((p) => p.name === 'spikePlugin').options
        if (opts.files.indexOf(f) < 0 && !util.isFileIgnored(f) && event !== 'addDir') {
  ...
  transformSpikeOptionsToWebpack (opts) {
    ...
    const disallow = ['output', 'resolveLoader', 'spike', 'plugins', 'afterSpikePlugins', 'context']
