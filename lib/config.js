...
module.exports = class Config {
  constructor (opts, project) {
    ...
    const spikeOpts = this.plugins.find((p) => p.id === 'spikeOptions')
    spikeOpts.project = project
  }
  ...
  validateOpts (opts) {
    ...
    res.server.files = [{
      ...
      fn: (event, file) => {
        ...
        const files = this.node.spike.files.all
  ...
  transformSpikeOptionsToWebpack (opts) {
    ...
    // TODO remove the reshape/babel/postcss from spike, they can be accessed
    // directly

    // TODO assign any extra options into node.spike.userOptions
    ...
    const util = new SpikeUtils(this)
    const spikePlugin = new SpikePlugin(util)
    spikePlugin.id = 'spikeOptions'
    spikePlugin.options = spike
    ...
    this.plugins = [
      ...
      new BrowserSyncPlugin(opts.server, { callback: (_, bs) => {
        if (bs.utils.devIp.length) {
          this.project.emit('info', `External IP: http://${bs.utils.devIp[0]}:${this.node.spike.server.port}`)
        }
      } })
    ]
    return this
