...
export default class Roots extends EventEmitter {
  ...
  static new (options) {
    ...
    // initialize the template, return validated options
    const sproutOptions = {
      locals: opts.locals,
      questionnaire: (prompts, skip) => {
        return W.promise((resolve, reject) => {
          const qs = prompts.filter((q) => { return skip.indexOf(q.name) > -1 })
    ...
    return promise
      ...
      .yield(opts)
      // should actually return an initialized project instance
  }
