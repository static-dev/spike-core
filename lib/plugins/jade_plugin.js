...
export default class JadeWebpackPlugin {
  ...
  apply (compiler) {
    ...
    compiler.plugin('make', (compilation, done) => {
      jadeFiles = compiler.options.roots.files.jade
      addFilesAsWebpackEntries(jadeFiles, this.opts, compilation)
        .then((res) => done())
    })
    ...
    compiler.plugin('emit', (compilation, done) => {
      jadeFiles.forEach((f) => {
        ...
        srcs.forEach((src) => {
          const p = resolveRelativeSourcePath(src, this.opts)
          dep.fileDependencies.push(p)
        })

        const outputPath = getOutputPath(f, this.opts).replace(/\.jade$/, '.html')
    ...
    compiler.plugin('compilation', (compilation) => {
      compilation.plugin('optimize-chunk-assets', (chunks, done) => {
        removeAssets(compilation, jadeFiles, this.opts, done)
      })
    })
  }
